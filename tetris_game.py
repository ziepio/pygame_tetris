...
def run_tetris_game():
    ...
    last_time_piece_moved = time.time()  # assigns time in sec since the Epoch
    piece = create_piece()
    score = 0
    while True:
        screen.fill((0, 0, 0))
    ...
    for row in range(5):
        for col in range(5):
            if shape_to_draw[row][col] != '.':
                draw_single_tetris_block(screen, piece['row'] + row, piece['column'] + col, (255, 255, 255),
                                         (217, 222, 226))
...
def draw_board(screen, matrix):
    game_matrix_column = 10
    game_matrix_row = 20
    for row in range(game_matrix_row):
        for column in range(game_matrix_column):
            if (matrix[row][column] != '.'):
                draw_single_tetris_block(screen, row, column, (255, 255, 255), (217, 222, 226))


def draw_single_tetris_block(screen, matrix_cell_row, matrix_cell_column, color, shadow_color):
...
def create_game_matrix():
    ...
    game_matrix_rows = 20
    matrix = []
    for rows in range(game_matrix_rows):
        new_row = []
        for columns in range(game_matrix_columns):
            new_row.append('.')
